...
_Partial 2026-07-21 — `audit_log` is append-only with a server-set actor/timestamp (`db/roles_and_rls.sql`); `db/audit_triggers.sql` captures EVERY insert/update/delete on the core tables (incl. direct-API changes) with the JWT actor + server clock; and `db/audit_hash_chain.sql` makes the log tamper-evident via hash chaining (`prev_hash`/`row_hash`, verified by `public.verify_audit_chain()`). Remaining: run all three SQL files in Supabase; for stronger immutability, optionally remove admin DELETE on `audit_log` and periodically export the latest `row_hash` off-box._
